...
export async function runAdapterConformance(adapterPath: string): Promise<boolean> {
  ...
  try {
    ...
    if (imported.default && typeof imported.default === 'function') {
      ...
      if (exports.length > 0) AdapterClass = exports[0];
    }
  ...
  try {
    ...
    if (devices.length === 0) {
      ...
      for (const d of devices) {
        if (!d.id || typeof d.id !== 'string') throw new Error('Device missing string "id".');
        if (!d.type || typeof d.type !== 'string') throw new Error(`Device ${d.id} missing string "type".`);
      }
  ...
  let capabilityNameForExecuteRaw: string | null = null;
  let deviceIdForExecute: string | null = null;
  ...
  for (const device of devices) {
    try {
      ...
      if (!Array.isArray(caps)) throw new Error(`getCapabilities for ${device.id} did not return an array.`);
      
      for (const cap of caps) {
        testedCapabilities++;
        if (!cap.name || typeof cap.name !== 'string') throw new Error('Capability missing string "name".');
  ...
  if (!deviceIdForExecute || !capabilityNameForExecuteRaw) {
    ...
    try {
      ...
      if (!result.schema_version) throw new Error("Missing 'schema_version' in result envelope.");
      if (!result.operation_id) throw new Error("Missing 'operation_id' in result envelope.");
      if (!['success', 'error'].includes(result.status)) throw new Error(`Invalid status: ${result.status}`);
      if (!result.completion || typeof result.completion.done !== 'boolean') throw new Error("Missing 'completion.done' boolean.");
      if (result.status === 'error' && (!result.error || !result.error.code)) throw new Error("Status is error but missing 'error.code' object.");
